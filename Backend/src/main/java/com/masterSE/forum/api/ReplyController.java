...
public class ReplyController implements RepliesApi {
	...
	@Override
	public ResponseEntity<ReplyResponse> updateReply(Long id, @Valid UpdateReply updateReply) {
		log.debug("Handling PUT /reply/{}", id);
		return replyService.update(id, updateReply)
				.map(ResponseEntity::ok)
				.orElseGet(() -> ResponseEntity.notFound().build());
	}
}
